...
#    @bot_dbaccess = DbAccess.current
  ...
  def test_read_all()
    @ramps_arduino.test_read_all("F82", false, @status_debug_msg)
  end
...
#     @ramps_param.check_parameters()
  ...
  def write_param_value(nr, val)
    @ramps_arduino.execute_command("F22 P#{nr.to_i} V#{val.to_i}", true, false)    
  end

  def read_param_value(nr)
    @ramps_arduino.execute_command("F21 P#{nr.to_i}", true, false)    
  end

  def read_all_params()
    @ramps_arduino.execute_command("F20", true, false)    
  end
